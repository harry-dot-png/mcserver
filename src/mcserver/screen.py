...
def screen_exists(name: str, pwd: Path = Path.cwd()) -> bool:
    query = subprocess.run("screen -list", capture_output=True, shell=True, cwd=pwd)
    return any(screen.group("name") == name for screen in SCREEN_PATTERN.finditer(query.stdout.decode()))
