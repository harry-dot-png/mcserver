...
def get_latest_stable_version_and_build() -> tuple[str, str]:
    ...
    with requests.get(f"https://api.papermc.io/v2/projects/paper/versions/{version}/builds") as response:
        data = response.json()

    stables = [build["build"] for build in data["builds"] if build["channel"] == "default"]
